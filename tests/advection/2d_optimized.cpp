/*
Optimized version of advection equation solver program for dccrg.
...
#include "adapter_optimized.hpp"
#include "cell.hpp"
#include "initialize_optimized.hpp"
#include "save.hpp"
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	// save initial state
	#ifndef DEBUG
	const string base_output_name("tests/advection/2d_optimized_");
	#else
	const string base_output_name("tests/advection/2d_optimized_debug_");
	#endif
	...
	double inner_solve_time = 0, outer_solve_time = 0;
	...
	while (time < tmax) {
		...
		const size_t solved_index = Solver().calculate_fluxes(cfl * dt, 0, grid);
		...
		Solver().calculate_fluxes(cfl * dt, solved_index + 1, grid);
