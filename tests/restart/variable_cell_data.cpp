...
public:
	...
	> get_mpi_datatype() const
	{
		if (Cell::transfer_all) {
			boost::array<int, 2> counts = {{1, int(this->data.size())}};
			boost::array<MPI_Aint, 2> displacements = {{
				0,
				(uint8_t*) &(this->data[0]) - (uint8_t*) &(this->data_size)
			}};
			if (this->data.size() == 0) {
				displacements[1] = 0;
			}
			boost::array<MPI_Datatype, 2> datatypes = {{MPI_UINT64_T, MPI_INT}};
			...
			return boost::make_tuple((void*) &(this->data_size), 1, final_datatype);

		} else if (Cell::transfer_data) {

			if (this->data.size() > 0) {
				return boost::make_tuple((void*) &(this->data[0]), int(this->data.size()), MPI_INT);
			} else {
				return boost::make_tuple((void*) &(this->data_size), 0, MPI_UINT64_T);
			}

		} else {
			return boost::make_tuple((void*) &(this->data_size), 1, MPI_UINT64_T);
		}
	}
...
int main(int argc, char* argv[])
{
	...
	*/
	const boost::array<uint64_t, 3> grid_length = {{20, 1, 1}};
	...
	if (not restart) {
		...
		boost::tuples::get<0>(header) = &header;
		boost::tuples::get<1>(header) = 0;
		boost::tuples::get<2>(header) = MPI_INT;
