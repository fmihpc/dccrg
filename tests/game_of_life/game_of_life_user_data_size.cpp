/*
Tests the grid with some simple game of life patters using cell data of fixed size, returns EXIT_SUCCESS if everything went ok.
*/
...
int main(int argc, char* argv[])
{
	...
	vector<uint64_t> all_cells = game_grid.get_cells();
	...
	basename << "game_of_life_data_size_" << comm.rank() << "_";
	ofstream outfile, visit_file;

	// visualize the game with visit -o game_of_life_test.visit
	if (comm.rank() == 0) {
		visit_file.open("game_of_life_data_size.visit");
	...
	for (int step = 0; step < TIME_STEPS; step++) {
		...
		if (comm.rank() == 0) {
			for (int process = 0; process < comm.size(); process++) {
				visit_file << "game_of_life_data_size_" << process << "_" << step_string.str() << suffix.str() << endl;
			}
		}
