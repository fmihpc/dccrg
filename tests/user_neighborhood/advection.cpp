...
template<class CellData> void initial_condition(Dccrg<CellData>& grid)
{
	...
	grid.update_remote_neighbor_data(NEIGHBORHOOD_ID);
}
...
template<class CellData> void get_neighbor_directions(
	...
	const vector<uint64_t>* neighbors = grid.get_neighbors(cell, NEIGHBORHOOD_ID);
	if (neighbors == NULL) {
		std::cerr << __FILE__ << ":" << __LINE__
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	while (time < tmax) {
		...
		grid.wait_neighbor_data_update_receives(NEIGHBORHOOD_ID);
